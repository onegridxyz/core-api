package xyz.onegrid.coreapi.web.rest.errors
...
class ExceptionTranslator(private val env: Environment) : ProblemHandling, SecurityAdviceTrait {
    ...
    @ExceptionHandler
    fun handleEmailAlreadyUsedException(ex: xyz.onegrid.coreapi.service.EmailAlreadyUsedException, request: NativeWebRequest): ResponseEntity<Problem>? {
    ...
    @ExceptionHandler
    fun handleUsernameAlreadyUsedException(ex: xyz.onegrid.coreapi.service.UsernameAlreadyUsedException, request: NativeWebRequest): ResponseEntity<Problem>? {
        val problem = LoginAlreadyUsedException()
        return create(problem, request, HeaderUtil.createFailureAlert(applicationName, false, problem.entityName, problem.errorKey, problem.message))
    }

    @ExceptionHandler
    fun handleInvalidPasswordException(ex: xyz.onegrid.coreapi.service.InvalidPasswordException, request: NativeWebRequest): ResponseEntity<Problem>? {
    ...
    private fun containsPackageName(message: String?) = listOf("org.", "java.", "net.", "javax.", "com.", "io.", "de.", "xyz.onegrid.coreapi").any { it == message }
}
